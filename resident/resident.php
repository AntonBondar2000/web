...
if (array_key_exists('user', $_SESSION)) {
    $log = "Пользователь: " . $_SESSION['user']['id'] . " - Зашел на страницу проживающих " . date('Y-m-d h:i:s A');
...
$residents = $mysql->query("
        SELECT booking.date_begin, booking.date_end, users.* FROM `booking` INNER JOIN `users` ON booking.id_user = users.id
");
$residents = $residents->fetch_all();
for($i = 0; $i < count($residents); $i++){
    if($residents[$i][0]>$now_data || $residents[$i][1] < $now_data){
        unset($residents[$i]);
    }
}
...
<body>
    ...
    <section class="resident">
        ...
        <div class="container wrap">
            <div class="row block-resident">


                <?php foreach($residents as $resident){ ?>
                    <div class="item-block-resident col-12">
                    <img src="../img/people_1.jpg" alt="Фотография">
                    <p class="fio"><?php echo $resident[3]; ?></p>
                    <p class="data"><?php echo $resident[0]; ?> : <?php echo $resident[1]; ?></p>
                </div>
